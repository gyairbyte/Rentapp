...
For installment plans, ensure the sum of all installment amounts equals the plan amount exactly to the cent. If equal division leaves a remainder, put the remainder in the final installment (for example, four installments totalling $1,756.51 should be $439.13, $439.13, $439.13, and $439.12). Store any per-installment late fee or penalty as a late_payment_terms entry inside that installment object, preserving the base installment amount and the rate (e.g. 0.10 for 10%).
Late fees, penalties, and conditional post-deadline amounts are not selectable payment options. Do not create a payment_options entry for a penalty or late fee. Instead, capture that information as late_payment_terms on the related base/full, discounted, or installment option or per-installment object. Each term should include term_type (penalty, late_fee, discount, or other), amount, rate (e.g. 0.10 for 10%), effective_date, due_date, and description. Do not collapse a multi-deadline notice into a single due date.`
}
